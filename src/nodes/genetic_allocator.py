...
class Genetic_Allocator():
    ...
    def __init__(self):
        ...
        # ROS Initialize
        rospy.init_node('genetic_allocator', anonymous=True) # Initialize Node
        ...
        self.Nr = 3
        self.Nt = 5
        self.p = np.array([[1, 2], [4, 5], [3, 1]]) 
        self.q =  np.array([[0, 0], [1, 0], [1, 1], [3, 3], [2, 0]]) 
        self.v = np.array([10, 14, 16])  
        self.q_p = np.array([[1, 1], [1, 0.5], [2, 1], [1, 2], [0.5, 1]]) 
        self.tau = np.array([3, 2, 2, 1, 2]) 
        self.lambda_penalty = np.array([5, 4, 2])  
        self.phi_penalty = np.array([5, 1, 3, 2, 2])   

        # Hyperparameters

        self.n_gen = 100
        self.r_mut = 0.8 # close to 1 but not 1
        self.r_cross = 0.8 # close to 1 but not 1
        self.pop_size = 100
        self.elite_frac = 0.1
        self.crossover_frac = 0.8

        self.allocation_result = PMRTAResult()

        # # Call function

        # self.best, self.score = self.ga(self.Nr, self.Nt, self.p, self.q, self.v, self.q_p, self.tau, self.lambda_penalty, self.phi_penalty, self.n_gen, self.pop_size, self.r_cross, self.r_mut, self.elite_frac, self.crossover_frac)
    ...
    def initializePublishers(self):
        ...
        self.pub_ga_allocation_result = rospy.Publisher("/ga_allocation_result", PMRTAResult, queue_size=10)
    ...
    def callback_problem(self, msg):
        ...
        print("GA Problem Received")
        # Update Problem Parameter
        self.Nr = msg.robot_number
        self.Nt = msg.task_number
        
        self.robot_position_x = msg.robot_position_x
        self.robot_position_y = msg.robot_position_y
        # self.p = np.array([[1, 2], [4, 5], [3, 1], [3, 8]]) 
        # print("Robot Position : ", self.p)
        self.p = np.transpose(np.array([(self.robot_position_x),(self.robot_position_y)]))
        # print("Robot Position : ", self.p)
        self.task_position_x = msg.task_position_x
        self.task_position_y = msg.task_position_y
        # self.q =  np.array([[0, 0], [1, 0], [1, 1], [3, 3], [2, 0]]) 
        self.q = np.transpose(np.array([(self.task_position_x),(self.task_position_y)]))
        self.v = np.array(msg.robot_velocity)  
        self.task_velocity_x = msg.task_velocity_x
        self.task_velocity_y = msg.task_velocity_y
        self.q_p = np.transpose(np.array([(self.task_velocity_x),(self.task_velocity_y)]))
        # self.q_p = np.array([[1, 1], [1, 0.5], [2, 1], [1, 2], [0.5, 1]]) 
        self.tau = np.array(msg.task_priority) 
        self.lambda_penalty = np.array(msg.robot_lambda)  
        self.phi_penalty = np.array(msg.task_operation_time) 
        ...
        self.allocation_result.allocation_result = self.best
        self.allocation_result.robot_number = self.Nr
        self.allocation_result.task_number = self.Nt

        self.pub_ga_allocation_result.publish(self.allocation_result)
    ...
    def ga(self,Nr, Nt, p, q, v, q_p, tau, lambda_penalty, phi_penalty, n_gen, pop_size, r_cross, r_mut, elite_frac, crossover_frac):
        # start min and mean
        ...
        # Initial pop
        ini_pop = self.ga_initial_pop(Nr, Nt, pop_size)
        # print(ini_pop) 
        pop_gen = list()
        ...
        for gen in range(n_gen):
            ...
            for sol in range(pop_size):
                ...
                # print(solution)
                J[sol] = self.objective_function(Nr, Nt, p, q, v, q_p, tau, lambda_penalty, phi_penalty, solution)
            ...
            sorted_actual_gen = self.sort_gen(actual_gen, index_J)
            ...
            for crossover_index in range(math.ceil(n_crossover_pop/2)):
                ...
                s1, s2 = self.ga_crossover(p1, p2, r_cross, Nr, Nt)
            ...
            for mutation_index in range(n_mutation_pop):
                p1_index = randint(0, n_elite_pop)
                p1 = elite_pop[p1_index]
                s1 = self.ga_mutation(p1, r_mut, Nr, Nt)
    ...
    # Initial population
    def ga_initial_pop(self,Nr, Nt, pop_size):
        ini_pop = list()
        tasks = list(range(Nt))
        tasks = self.incr(tasks, 1)
    ...
    def objective_function(self,Nr, Nt, p, q, v, q_p, tau, lambda_penalty, phi_penalty, solution):
        J = 0
        J_dist = 0
        J_time = 0
        moving_q = q.copy()
        ...
        for robot in range(Nr):
            ...
            t_ell = 0
            if first_task != 0:
                ell, t_ell, p_p = self.interception(p[robot,:2], moving_q[first_task-1,:2], v[robot], q_p[first_task-1,:2])
            ...
            for task in range(Nt-1): 
                ...
                if next_task != 0:
                    ...
                    ell, t_ell, p_p = self.interception(prev_ell, moving_q[next_task-1,:2], v[robot], q_p[next_task-1,:2])
    ...
    def ga_crossover(self,p1_input, p2_input, r_cross, Nr, Nt): 
        p1 = p1_input.copy()
        p2 = p2_input.copy()
        # Split parents
    ...
    # mutation operator
    def ga_mutation(self,p1_input, r_mut, Nr, Nt):
        p1 = p1_input.copy()
    ...
    # increment list
    def incr(self,list, i):
        return [x+i for x in list]

    # Interception point
    def interception(self,p, q, v, q_p):
        deltax = q[0] - p[0]
        deltay = q[1] - p[1]
        t_ell = float('inf')
